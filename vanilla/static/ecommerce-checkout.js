...
const COUNTRIES = {
  BR: {
    name: 'Brazil', flag: '🇧🇷', currency: 'BRL', language: 'pt',
...
function init() {
  ...
  $$('#country-selector .country-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (country !== state.country && !state.isLoadingCheckout) onCountryChange(country);
    });
  });

  $('pay-button').addEventListener('click', onPayClick);
  $('btn-continue-shopping').addEventListener('click', () => location.reload());
  $('btn-retry').addEventListener('click', () => { hideOverlay('error'); state.isPaying = false; });
...
async function loadCheckoutForCountry(country) {
  ...
  try {
    ...
    const session = await fetch(`/checkout/sessions?country=${country}`, { method: 'POST' }).then((r) => r.json());

    if (!session.checkout_session) throw new Error('Invalid session response');
    ...
    await state.yuno.startCheckout({
      ...
      onLoading: ({ isLoading }) => {
        if (!state.isPaying) {
          ...
          else { hideSkeleton(); $('yuno-root').classList.remove('loading'); setPayButtonState(true); }
        }
      },

      renderMode: {
        type: 'modal',
        elementSelector: { apmForm: '#yuno-form', actionForm: '#yuno-action-form' },
      },

      card: {
        type: 'extends',
        styles: `* { font-family: 'Inter', -apple-system, sans-serif !important; }`,
      },
      ...
      async yunoCreatePayment(oneTimeToken) {
        ...
        try {
          const resp = await fetch(`/payments?country=${state.country}`, {
            ...
            body: JSON.stringify({ oneTimeToken, checkoutSession: state.checkoutSession }),
          }).then((r) => r.json());

          if (resp.error || resp.status === 'ERROR') throw new Error(resp.error_message || 'Payment API error');
          ...
          state.isPaying = false;
          $('error-message-text').textContent = 'An unexpected error occurred. Please try again.';
          showOverlay('error');
        }
      },
      ...
      yunoPaymentResult(data) {
        ...
        switch (status) {
          case 'SUCCEEDED': case 'VERIFIED': showSuccessState(); break;
          case 'PENDING': showOverlay('pending'); break;
          case 'REJECTED': showErrorState('Your payment was rejected. Please try a different payment method or contact your bank.'); break;
          case 'DECLINED': showErrorState('Your payment was declined. Please verify your card details and try again.'); break;
          case 'ERROR': showErrorState('A payment error occurred. Please try again.'); break;
          case 'CANCELLED': case 'CANCELED_BY_USER': setPayButtonState(true); break;
          default: setPayButtonState(true);
        }
      },
      ...
      yunoError(error) {
        ...
        if (errCode === 'CANCELED_BY_USER' || errCode === 'CANCELLED') { setPayButtonState(true); return; }
...
function onPayClick() {
  if (state.yuno && !state.isPaying && !state.isLoadingCheckout) state.yuno.startPayment();
}
...
function updateOrderTotals(country) {
  ...
  if (!t) return;
  CART_ITEMS.forEach((item, i) => { const el = $(`item-price-${i}`); if (el) el.textContent = item.prices[country]; });
...
function updateMethodChips(country) {
  ...
  if (!chipsEl) return;
  chipsEl.innerHTML = cfg.methods.map((m, i) => {
...
function showSkeleton() { $('skeleton-wrap')?.classList.add('show'); }
function hideSkeleton() { $('skeleton-wrap')?.classList.remove('show'); }
function setPayButtonState(enabled) { const btn = $('pay-button'); if (btn) btn.disabled = !enabled; }
function showOverlay(type) { $(`overlay-${type}`)?.classList.add('show'); }
function hideOverlay(type) { $(`overlay-${type}`)?.classList.remove('show'); }
...
function showToast(message, type = 'info') {
  ...
  toast.innerHTML = `<svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true"><circle cx="7" cy="7" r="6" stroke="currentColor" stroke-width="1.4"/><path d="M7 4.5v3M7 9.5v.5" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg><span>${message}</span>`;
  container.appendChild(toast);
  requestAnimationFrame(() => toast.classList.add('in'));
  const remove = () => { toast.classList.remove('in'); toast.classList.add('out'); setTimeout(() => toast.remove(), 300); };
...
function onPromoApply() {
  ...
  if (!val) return;
  if (val.toUpperCase() === 'ELARA10') showToast('Promo code applied! 10% off your order.', 'success');
  else showToast('Invalid promo code. Try ELARA10 for 10% off.', 'error');
}
